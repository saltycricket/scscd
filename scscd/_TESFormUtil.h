...
/*
	This is taken from RE/T/TESFormUtil.h to avoid having to pull in every form.
	We disabled anything that isn't explicitly pulled in by _fallout.h.
	If you need a cast and it's not working as it should, look at this code.
*/
...
namespace RE
{
	...
	[[nodiscard]] const T* TESForm::As() const noexcept
	{
		...
		logger::warn(std::format("could not perform cast, returning null. Form type was {:#04x}. Check _TESFormUtil.h", (int32_t) GetFormType()));
		return nullptr;
	}
}
